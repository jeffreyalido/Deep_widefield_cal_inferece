...
nt = 200;  % frames
% nt = 2;  % frames JUST FOR DEBUGGING
fn = 10; % frame rate
pavg = 5; %mW per mm^2. for noisy measurements
spike_opts.rate = pos_spikerates(idx);
...
vessel_mod.seed = 10; 
%% output path
output_dir = sprintf('/projectnb/tianlabdl/caragon/Data/%s', ...
                                             spike_opts.rate)
% % make sub folders
% buf = true;
% id = 1;
...
saveastiff(im2uint16(neur_vol / max(neur_vol(:))), sprintf('%s/neur_vol.tiff', output_dir));

neur_ves = vol_out.neur_ves_all;
saveastiff(im2uint16(neur_ves / max(neur_ves(:))), sprintf('%s/neur_ves_all.tiff', output_dir));
%% generate PSFs
tic
