...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Parse inputs
file_path = sprintf('/projectnb/tianlabdl/caragon/psfs/cm2v2/cm2v2_interpolated_stack-%d.tif',view_ind);
yo = tiff_reader(file_path);
yo = yo(:,:,1:100);
for i = 1 : 100
...
% get net vessel volume
if vessel_mod.flag
top =(N3 -  size(PSF_struct.psf, 3)) / 2;
valid_vessel = neur_vol.neur_ves(:, :, top + 1 : top + Np3);
se = strel('sphere', 8);
% only dilate large ones
valid_vessel = imopen(valid_vessel, se);

rng(123)
dilate_trace = rand((vessel_mod.FOV_N +1)* (vessel_mod.FOV_N +1)* (vessel_mod.FOV_N+1), ...
                    round(Nt / (vessel_mod.frate / vessel_mod.vessel_dynamics)));
dilate_trace = imresize(dilate_trace, [(vessel_mod.FOV_N+1)* (vessel_mod.FOV_N+1)* (vessel_mod.FOV_N+1), Nt]);
dilate_trace(dilate_trace < 0) = 0;
end
...
%         clear TMPvol; 
        ...
        %% vessel volume
        if vessel_mod.flag
            vessel_img = vessel_dilation(valid_vessel, vessel_mod.FOV_N, vessel_mod.seed, vessel_mod.max_dilate_amp, dilate_trace, kk);
        else
            vessel_img = ones(size(WMPvol_w_bg(:, :, z_loc:(z_loc+Np3-1))));
        end
        ...
        clean_img_wo_bg = single_scan((WMPvol_w_bg(:,:,...
                                        z_loc:(z_loc+Np3-1)) + f0vol_wo_bg(:,:,z_loc:(z_loc+Np3-1))) .* vessel_img, ...
                                        size(PSF), freq_psf, scan_avg, true); 
...
%         % dend
        ...
        clean_img_w_bg = (sigscale/(2*sfrac^2))*single_scan((WMPvol_w_bg(:,:,...
                  z_loc:(z_loc+Np3-1))+f0vol(:,:,z_loc:(z_loc+Np3-1))) .* vessel_img,...
